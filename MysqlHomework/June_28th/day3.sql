...
alter table tb_emp add constraint emp_dept_fk foreign key(id) references tb_dept(deptId);
...
create table tb_emp1(
    ...
    name   VARCHAR(25),
    deptId INT(11)
    constraint fk foreign key(id) references tb_dept(id),  
    salary FLOAT)
...
select f_name from fruits,suppliers where f_id='a1'and suppliers.s_id=fruits.s_id;
...
-- 在customers表和orders表中，使用INNER JOIN语法查询customers表中ID为10001的客户的订单信息
 select * from customers join orders on customers.c_id=1000;
-- 查询fruits表所有信息附加一列suppliers的供应商名称，使用INNER JOIN语法进行内连接查询，并对查询结果排序
select suppliers.s_name from suppliers join fruits on fruits.s_id=suppliers.s_id order by s_name desc;
...
select c_name,grade from student,score where score.stu_id=student.id and name='李四';
-- 查询所有学生的信息和考试信息
select * from student,score where student.id=score.stu_id;
-- 计算每个学生的总成绩
select * from student,(select stu_id,sum(grade) from score group by stu_id) t2 where student.id=t2.stu_id;
-- 计算每个考试科目的平均成绩
select c_name,avg(grade) from score group by c_name;
-- 查询计算机成绩低于95的学生信息
select * from student ,(select * from score where grade<95) t1 where t1.stu_id=student.id and t1.c_name='计算机';
-- 查询同时参加计算机和英语考试的学生的信息

-- 从student表和score表中查询出学生的学号，然后合并查询结果
select distinct student.name,student.id from student,score where student.id=score.stu_id;
-- 查询姓张或者姓王的同学的姓名、院系和考试科目及成绩
select name,department,score.c_name,grade from student,score where student.id=score.stu_id and name like'张%' or name like'王%';
-- 查询都是湖南的学生的姓名、年龄、院系和考试科目及成绩
select name,birth,department,c_name,grade from student,score where student.id=score.stu_id and student.address like'湖南%';
